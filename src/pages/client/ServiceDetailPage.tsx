...
import GameServerDetail from '../../components/services/game-server/GameServerDetail';
...
const ServiceDetailPage = () => {
  ...
  // ── Game server — vista especializada ──────────────────────
  if (svc?.is_game_server) {
    return <GameServerDetail service={svc} />;
  }

  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <ServiceDetailHeader service={svc} />

      <div className="grid grid-cols-1 lg:grid-cols-5 gap-8 mt-8">
        {/* Columna Izquierda */}
        <div className="lg:col-span-3 space-y-8">
          <GeneralInfoCard service={svc} />
          <InfoCard title="Historial de Pagos" subtitle="Movimientos recientes de este servicio">
            <PaymentHistory serviceId={svc?.uuid} />
          </InfoCard>
          <QuickActionsCard service={svc} />
        </div>

        {/* Columna Derecha */}
        <div className="lg:col-span-2 space-y-8">
          <SpecificationsCard specs={svc?.plan?.specifications} />
          <PerformanceCard metrics={svc?.metrics} />
