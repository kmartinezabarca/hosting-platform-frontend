...
const NewDashboard = () => {
  ...
  const services = (servicesResponse as any) || [];
  const activity = (activityResponse as any) || [];
          ...
          <div className="flex items-center justify-between">
            <div className="space-y-2">
              <h1 className="text-3xl font-bold text-foreground">
                ...
                {user?.first_name || (user as any)?.name || "Usuario"}!
              </h1>
          ...
          {services.length > 0 ? (
            <ul className="space-y-2">
              {services.slice(0, 3).map((s) => {
                const type = mapCategoryToType((s as any).category_slug ?? s.type);
