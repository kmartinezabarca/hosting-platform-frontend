    ...
    <div className="space-y-8 p-6 bg-background min-h-screen">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between space-y-4 lg:space-y-0">
        <div className="space-y-1">
          ...
          <p className="text-muted-foreground text-lg">Panel de control y métricas de la plataforma</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
        ...
        <Card className="relative overflow-hidden border-0 shadow-lg bg-gradient-to-br from-blue-50 to-blue-100 dark:from-blue-950/50 dark:to-blue-900/30">
          ...
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-3">
            <CardTitle className="text-sm font-medium text-blue-900 dark:text-blue-100">Usuarios Totales</CardTitle>
              ...
              <Progress
                value={
                  stats?.data?.users?.total
                    ? ((stats?.data?.users?.active || 0) / stats?.data?.users?.total) * 100
                    : 0
                }
        ...
        <Card className="relative overflow-hidden border-0 shadow-lg bg-gradient-to-br from-emerald-50 to-emerald-100 dark:from-emerald-950/50 dark:to-emerald-900/30">
          ...
          <CardContent className="space-y-3">
            ...
            <div className="space-y-2">
              ...
              <Progress
                value={
                  stats?.data?.services?.total
                    ? ((stats?.data?.services?.active || 0) / stats?.data?.services?.total) * 100
                    : 0
                }
          ...
          <CardContent className="space-y-3">
            ...
            <div className="flex items-center space-x-2">
              ...
              <span className="text-xs text-purple-700 dark:text-purple-300">
                Respuesta: {stats?.data?.tickets?.avg_response_time || '0 horas'}
              </span>
            </div>
            <div className="space-y-2">
              <div className="flex justify-between text-xs text-purple-700 dark:text-purple-300">
                <span className="text-red-600 dark:text-red-400">
                  Urgentes: {formatNumber(stats?.data?.tickets?.urgent)}
                </span>
                <span className="text-orange-600 dark:text-orange-400">
                  Alta: {formatNumber(stats?.data?.tickets?.high_priority)}
                </span>
              </div>
              <div className="flex space-x-1">
                <div className="flex-1 bg-red-200 dark:bg-red-800 h-2 rounded">
                  <div
                    ...
                    style={{
                      width: `${
                        stats?.data?.tickets?.open
                          ? ((stats?.data?.tickets?.urgent || 0) / stats?.data?.tickets?.open) * 100
                          : 0
                      }%`,
                    }}
                  />
                ...
                <div className="flex-1 bg-orange-200 dark:bg-orange-800 h-2 rounded">
                  <div
                    ...
                    style={{
                      width: `${
                        stats?.data?.tickets?.open
                          ? ((stats?.data?.tickets?.high_priority || 0) / stats?.data?.tickets?.open) * 100
                          : 0
                      }%`,
                    }}
                  />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="border-0 shadow-lg lg:col-span-2">
          ...
          <CardContent>
            <div className="space-y-3">
              {stats?.data?.recent_activity?.slice(0, 6)?.length ? (
