...
const AdminBlogPage = () => {
  ...
  const [confirmModal, setConfirmModal] = useState<{ isOpen: boolean; post: any }>({ isOpen: false, post: null });
  const [isActionLoading, setIsActionLoading] = useState(false);
  const [isDeleting, setIsDeleting] = useState(false);

  // Sheet / form state (kept for legacy handlers; sheet UI was removed but handlers still reference these)
  const [isSheetOpen, setIsSheetOpen] = useState(false);
  const [editingPost, setEditingPost] = useState<any>(null);
  const [imagePreview, setImagePreview] = useState<any>(null);
  const [formData, setFormData] = useState<any>({
    title: '', slug: '', content: '', excerpt: '',
    category_id: '', image: '', is_featured: false, is_published: true
  });
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      let allPosts: any[] = [];
      if ((postsRes.data as any)?.data && Array.isArray((postsRes.data as any).data)) {
        allPosts = (postsRes.data as any).data;
        const total = (postsRes.data as any).total || allPosts.length;
        const lastPage = (postsRes.data as any).last_page || Math.ceil(total / perPage) || 1;
        setTotalPages(lastPage);
      } else if (Array.isArray(postsRes.data as any)) {
        allPosts = postsRes.data as any;
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      const message = (error as any)?.response?.data?.message || (error as any)?.message || 'Error al eliminar';
  ...
  const handleImageUpload = async (e) => {
    ...
    reader.onload = (ev) => {
      setImagePreview((ev.target as FileReader)?.result as string);
    };
