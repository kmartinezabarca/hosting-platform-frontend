...
import apiDocumentationAdminService from "@/services/apiDocumentationAdminService";
...
export default function AdminApiDocumentationPage() {
  ...
  const fetchDocumentation = async () => {
    try {
      setLoading(true);
      const data = await apiDocumentationAdminService.getAll();
  ...
  const handleSave = async (e) => {
    ...
    try {
      if (editingDoc) {
        await apiDocumentationAdminService.update(editingDoc.uuid, formData);
        toast.success(`Documentación de API "${formData.title}" actualizada`, {
          description: "Los cambios han sido guardados.",
        });
      } else {
        await apiDocumentationAdminService.create(formData);
        toast.success(`Documentación de API "${formData.title}" creada`, {
      ...
      console.error(error);
      const message = error?.response?.data?.message || "Error al guardar la documentación de API";
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      await apiDocumentationAdminService.delete(confirmModal.doc.uuid);
      toast.success("Documentación de API eliminada", {
      ...
      console.error(error);
      toast.error("Error", { description: "Error al eliminar la documentación de API" });
    } finally {
  ...
  return (
    <div className="space-y-6 p-6 max-w-[1600px] mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">Documentación de API</h1>
          <p className="text-sm text-muted-foreground mt-1">{stats.total} endpoints registrados</p>
        </div>
        ...
        <CardContent className="p-4">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
            ...
            <div className="text-sm text-muted-foreground">
              <span className="font-medium text-foreground">{filteredDocs.length}</span> endpoints
            </div>
          </div>
                ...
                {isLoadingState ? (
                  ...
                  <tr>
                    <td colSpan={4} className="px-4 py-12 text-center">
                      ...
                      <p className="text-sm text-muted-foreground">No se encontró documentación de API</p>
                    </td>
                  </tr>
                ) : (
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center justify-end gap-1">
                          <Tooltip>
                            ...
                            <TooltipContent>Editar endpoint</TooltipContent>
                          </Tooltip>
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <Button
                            ...
                            <TooltipContent>Eliminar endpoint</TooltipContent>
                          </Tooltip>
                        </div>
                      </td>
                    </tr>
          ...
          <div className="flex flex-col h-full">
            <SheetHeader className="px-6 py-4 border-b shrink-0">
              <SheetTitle className="text-xl font-semibold">
                {editingDoc ? "Editar Endpoint" : "Nuevo Endpoint de API"}
              </SheetTitle>
              <SheetDescription>
                {editingDoc ? "Modifica los datos del endpoint" : "Completa la información para crear un nuevo endpoint"}
              </SheetDescription>
            </SheetHeader>
              ...
              <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    className="h-10"
                    placeholder="Categoría del endpoint"
                  />
                </div>
                ...
                <div className="flex gap-3">
                  ...
                  <Button type="submit" className="flex-1" disabled={isSubmitting}>
                    {isSubmitting ? (
                      ...
                      editingDoc ? "Guardar Cambios" : "Crear Endpoint"
                    )}
                  </Button>
                </div>
              </div>
      ...
      >
        <p>¿Estás seguro de que quieres eliminar el endpoint <strong>{confirmModal.doc?.title}</strong>? Esta acción no se puede deshacer.</p>
      </ConfirmationModal>
    </div>
  );
}
