...
const AdminServicePlansPage = () => {
  ...
  const validateForm = () => {
    ...
    if (!formData.base_price || parseFloat(formData.base_price) <= 0) {
      errors.base_price = 'El precio base debe ser mayor a 0';
    }
  ...
  return (
    <div className="space-y-6 p-6 max-w-[1600px] mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">Planes de Servicio</h1>
          <p className="text-sm text-muted-foreground mt-1">{stats.total} planes registrados</p>
        </div>

        <div className="flex items-center gap-2">
          <Button 
            onClick={() => { setDataLoaded(false); refetchPlans(); }} 
            variant="outline" 
            size="sm"
          ...
          <Button onClick={openCreateSheet} size="sm" disabled={plansLoading}>
                    ...
                    <tr key={plan.id || plan.uuid} className="hover:bg-muted/30 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1">
                          ...
                          <span className="font-semibold text-sm">{plan.base_price}</span>
            ...
            <div className="flex items-center justify-between px-4 py-3 border-t">
              ...
              <div className="flex items-center gap-2">
                <Button
                  ...
                  disabled={currentPage === 1 || plansLoading}
                >
                  Anterior
                </Button>
                <div className="flex items-center gap-1">
                ...
                <Button
                  ...
                  disabled={currentPage === totalPages || plansLoading}
                >
                  Siguiente
                </Button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto px-6 py-4">
                <Tabs defaultValue="basic" className="w-full h-full">
                  ...
                  <TabsContent value="basic" className="space-y-4">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      ...
                      <div className="space-y-2">
                        ...
                        <Input
                          id="slug"
                          value={formData.slug}
                          onChange={(e) => { setFormData(p => ({ ...p, slug: e.target.value })); setFormErrors(p => ({ ...p, slug: '' })); }}
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div className="space-y-2">
                        ...
                        <Select value={formData.category_id} onValueChange={(v) => { setFormData(p => ({ ...p, category_id: v })); setFormErrors(p => ({ ...p, category_id: '' })); }}>
                      ...
                      <div className="space-y-2">
                        <Label htmlFor="base_price" className="text-sm font-medium">Precio Base (MXN) *</Label>
                        <Input
                          id="base_price"
                          type="number"
                          value={formData.base_price}
                          onChange={(e) => { setFormData(p => ({ ...p, base_price: e.target.value })); setFormErrors(p => ({ ...p, base_price: '' })); }}
                          className="h-10"
                          placeholder="99.00"
                        />
                        {formErrors.base_price && <p className="text-xs text-red-500">{formErrors.base_price}</p>}
                      </div>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-6">
                      <div className="flex items-center gap-3">
                        <Switch
                          ...
                          onCheckedChange={(checked) => setFormData(p => ({ ...p, is_popular: checked }))}
                        />
                        <Label htmlFor="is_popular" className="text-sm font-medium cursor-pointer">Plan Popular</Label>
                      </div>
                      <div className="flex items-center gap-3">
                        <Switch
                          id="is_active"
                        ...
                        />
                        <Label htmlFor="is_active" className="text-sm font-medium cursor-pointer">Plan Activo</Label>
                      </div>
                    </div>
                  </TabsContent>
                  ...
                  <TabsContent value="pricing" className="space-y-3">
                    <div className="flex justify-between items-center shrink-0">
                      <Label className="text-sm font-medium">Precios por Ciclo</Label>
                    ...
                    <div className="space-y-2 max-h-[calc(90vh-280px)] overflow-y-auto pr-1">
                      {formData.pricing.map((pricing, index) => (
                        <div key={index} className="flex gap-2">
                          <Select value={pricing.billing_cycle_id} onValueChange={(v) => updatePricing(index, 'billing_cycle_id', v)}>
      ...
      >
        <p>¿Estás seguro de que quieres eliminar el plan <strong>{confirmModal.plan?.name}</strong>? Esta acción no se puede deshacer.</p>
      </ConfirmationModal>
    </div>
  );
};
