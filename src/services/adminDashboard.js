...
const adminDashboardService = {
  ...
  getStats: async (period = 'month') => {
    const response = await apiClient.get(`/admin/dashboard/stats?period=${period}`);
  ...
  getServicesDistribution: async () => {
    const response = await apiClient.get('/admin/dashboard/services-distribution');
    return response.data;
  },

  // Obtener distribución de tickets por prioridad
  getTicketsPriority: async () => {
    const response = await apiClient.get('/admin/dashboard/tickets-priority');
    return response.data;
  },
  ...
  getUsersMetrics: async (period = 'month') => {
    const response = await apiClient.get(`/admin/dashboard/users-metrics?period=${period}`);
    return response.data;
  },

  // Obtener métricas de servicios
  getServicesMetrics: async (period = 'month') => {
    const response = await apiClient.get(`/admin/dashboard/services-metrics?period=${period}`);
    return response.data;
  },

  // Obtener métricas de ingresos
  getRevenueMetrics: async (period = 'month') => {
    const response = await apiClient.get(`/admin/dashboard/revenue-metrics?period=${period}`);
    return response.data;
  },

  // Obtener métricas de tickets
  getTicketsMetrics: async (period = 'month') => {
    const response = await apiClient.get(`/admin/dashboard/tickets-metrics?period=${period}`);
    return response.data;
  }
};
