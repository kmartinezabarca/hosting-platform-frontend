...
import StatusFilterDropdown from '@presentation/components/features/filters/StatusFilterDropdown';
import {
  Receipt,
  Eye,
  DollarSign,
 ...
 */
/* ── CFDI status badge + download buttons ──────────── */
function CfdiStatusBadge({ invoice, onAddFiscal }) {
  const status = invoice.cfdi_status;
  const downloadMut = useDownloadCfdi();

  if (!status || status === 'not_required') return null;

  const hoursLeft = invoice.stamp_scheduled_at
    ? Math.max(0, Math.round((new Date(invoice.stamp_scheduled_at).getTime() - Date.now()) / 3_600_000))
    : null;

  if (status === 'stamped') {
    return (
      <div className="flex items-center gap-2 flex-wrap mt-2">
        <span className="inline-flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-full bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 font-medium border border-emerald-500/20">
          <CheckCircle className="w-3 h-3" />
          CFDI Timbrado
        </span>
        <button
          onClick={() => {
            downloadMut.mutate({ uuid: invoice.uuid, format: 'pdf', invoiceNumber: invoice.invoice_number });
          }}
          disabled={downloadMut.isPending}
          className="inline-flex items-center gap-1 text-xs px-2.5 py-1 rounded-full border border-border hover:bg-muted transition-colors disabled:opacity-50"
        >
          <Download className="w-3 h-3" />PDF
        </button>
        <button
          onClick={() => {
            downloadMut.mutate({ uuid: invoice.uuid, format: 'xml', invoiceNumber: invoice.invoice_number });
          }}
          disabled={downloadMut.isPending}
          className="inline-flex items-center gap-1 text-xs px-2.5 py-1 rounded-full border border-border hover:bg-muted transition-colors disabled:opacity-50"
        >
  ...
  if (status === 'scheduled' || status === 'needs_info') {
    return (
      <div className="flex items-center gap-2 flex-wrap mt-2">
        <span className="inline-flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-full bg-amber-500/10 text-amber-700 dark:text-amber-400 font-medium border border-amber-500/20">
          <Clock className="w-3 h-3" />
          {hoursLeft !== null ? `Pendiente (${hoursLeft}h restantes)` : 'Pendiente de timbrado'}
        </span>
        <button
          onClick={onAddFiscal}
          className="inline-flex items-center gap-1 text-xs px-2.5 py-1 rounded-full bg-foreground text-background hover:opacity-90 transition-opacity font-medium"
        >
          Agregar RFC
        </button>
      </div>
  ...
  if (status === 'pending_stamp') {
    return (
      <div className="mt-2">
        <span className="inline-flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-full bg-blue-500/10 text-blue-700 dark:text-blue-400 font-medium border border-blue-500/20">
  ...
  if (status === 'failed') {
    return (
      <div className="mt-2">
        <span
          title={invoice.cfdi_error}
          className="inline-flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-full bg-red-500/10 text-red-700 dark:text-red-400 font-medium border border-red-500/20 cursor-help"
        >
  ...
  if (status === 'cancelled') {
    return (
      <div className="mt-2">
        <span className="inline-flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-full bg-muted text-muted-foreground font-medium border border-border">
          Cancelado
        </span>
      </div>
    );
...
const InvoicesList = ({ invoices, filters, setFilters, onSelectInvoice, onPayInvoice }) => {
  ...
  return (
    <div className="space-y-6">
          ...
          <AnimatePresence>
            {filteredInvoices.map((invoice, index) => (
              ...
              >
                ...
                <CfdiStatusBadge
                  invoice={invoice}
                  onAddFiscal={() => setFiscalDataInvoice(invoice)}
                />
      ...
      {/* Modal de datos fiscales — ventana 72h */}
      {fiscalDataInvoice && (
        <UpdateFiscalDataModal
          invoice={fiscalDataInvoice}
          onClose={() => setFiscalDataInvoice(null)}
        />
      )}
    </div>
  );
