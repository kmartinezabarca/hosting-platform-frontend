...
export default function AdminApiDocumentationPage() {
  ...
  const onSubmit = async (data) => {
    try {
      if (editingDoc) {
        await apiDocumentationAdminService.update(editingDoc.uuid, data);
        toast.success(`Documentación de API "${data.title}" actualizada`);
      } else {
        await apiDocumentationAdminService.create(data);
        toast.success(`Documentación de API "${data.title}" creada`);
      }
      closeForm();
      fetchDocumentation();
    } catch (error) {
      console.error(error);
      const message = (error as any)?.response?.data?.message || "Error al guardar la documentación de API";
  ...
  if (showForm) {
    return (
      <div className="px-6 py-4">
        <div className="flex items-center justify-between mb-6 max-w-7xl mx-auto">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-lg font-semibold">
              {editingDoc ? "Editar Documentación de API" : "Nueva Documentación de API"}
            </h1>
          </div>
          <div className="flex items-center gap-2">
            ...
            <Button type="button" onClick={() => (document.getElementById("api-doc-form") as HTMLFormElement | null)?.requestSubmit()} disabled={isSubmitting} size="sm">
              {isSubmitting ? (
        ...
        <form id="api-doc-form" onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-1 lg:grid-cols-3 gap-6 max-w-7xl mx-auto">
              ...
              <CardContent className="p-4 space-y-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <Input
                    ...
                    {...register("category")}
                    className="h-9"
                    placeholder="Categoría (ej: Autenticación, Pagos)"
                  />
                </div>
              </CardContent>
            </Card>
    ...
    <div className="space-y-6 p-6 max-w-[1600px] mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">{stats.total} endpoints/docs registrados</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <Button onClick={openNewForm} size="sm" disabled={loading}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <StatCard icon={FileCode} label="Total"      value={stats.total}    accent="slate"   loading={loading} />
      ...
      <ConfirmationModal
        ...
        title="Eliminar Documentación de API"
        confirmText="Eliminar"
        isConfirming={isActionLoading}
      >
        <p>¿Estás seguro de que quieres eliminar la documentación de API <strong>{confirmModal.doc?.title}</strong>? Esta acción no se puede deshacer.</p>
      </ConfirmationModal>
    </div>
  );
}
