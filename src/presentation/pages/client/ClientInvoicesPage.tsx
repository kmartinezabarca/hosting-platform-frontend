...
const ClientInvoicesPage = () => {
  ...
  const processPayment = (invoiceId: any, paymentMethodId: any) => {
    processPaymentMutation.mutate(
      { invoice_id: invoiceId, payment_method_id: paymentMethodId } as any,
      {
        onSuccess: () => {
          setShowPaymentModal(false);
          toast.success('Tu pago se está procesando. La factura se actualizará en breve.');
        },
        onError: (error: any) => {
          toast.error(error?.message || 'No se pudo procesar el pago. Inténtalo de nuevo.');
        },
      }
    );
  };

  const handleSetDefaultPaymentMethod = (method: any) => {
    setDefaultMethodMutation.mutate(method.uuid, {
      onSuccess: () => {
        toast.success(`La tarjeta que termina en ${method.last4} es ahora tu método predeterminado.`);
      },
      onError: (error: any) => {
        toast.error(error?.message || 'No se pudo establecer el método como predeterminado.');
      },
    });
  };
  ...
  const confirmDelete = () => {
    if (methodToDelete) {
      deleteMethodMutation.mutate(methodToDelete.uuid, {
        onSuccess: () => {
          toast.success(`La tarjeta que termina en ${methodToDelete.last4} ha sido eliminada.`);
          setMethodToDelete(null);
        },
        onError: (error: any) => {
          toast.error(error?.message || 'No se pudo eliminar el método de pago.');
          setMethodToDelete(null);
        },
      });
    }
  ...
  const handlePaymentMethodAdded = () => {
    ...
    toast.success('Tu nuevo método de pago está listo para usarse.');
  };
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <PaymentModal
        ...
        {...({ isProcessing: processPaymentMutation.isPending } as any)}
      />
      <InvoiceDetailModal
