// StripeCheckout.tsx - VERSIÓN COMPLETA Y CORREGIDA
...
interface ServiceData {
  ...
  invoiceConstanciaB64?: string;
  invoiceConstanciaName?: string;
  invoiceConstanciaMime?: string;
  [key: string]: unknown;
    ...
    const createIntentForNewCard = async () => {
      try {
        ...
        if ((response as any)?.success) {
          setClientSecret((response as any).data.client_secret);
        } else {
          onError?.((response as any)?.message || "Error creando el intento de pago");
        }
      } catch (error) {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    try {
      ...
      if (paymentMethodId) {
        ...
        const contractResponse: any = await servicesService.contractService({
          plan_id: serviceData.plan_id as any,
      ...
      onError?.((error as any)?.message || "El procesamiento del pago falló.");
    } finally {
      setProcessing(false);
    }
  };
