      ...
      <div className="relative z-10 w-full max-w-sm bg-background rounded-2xl border border-border shadow-2xl p-6">
        ...
        <div className="flex gap-3">
          ...
          <button
            ...
            disabled={isPending}
            className={cn(
              'flex-1 py-2 rounded-xl text-sm font-semibold transition-opacity disabled:opacity-50 flex items-center justify-center gap-2',
...
const STATE_LABELS: Record<string, { label: string; color: string; dot: string }> = {
  ...
  starting: { label: 'Iniciando…',     color: 'text-blue-400',    dot: 'bg-blue-400 animate-pulse' },
  ...
  const handleAction = (signal: PowerSignal) => {
    if (signal === 'stop') {
      setConfirmModal({
        ...
        description: '¿Estás seguro? Los jugadores conectados serán desconectados.',
        danger: false,
      });
    } else if (signal === 'kill') {
      setConfirmModal({
        ...
        description: 'Apagado inmediato — puede corromper datos no guardados. Solo en emergencias.',
        danger: true,
      });
    } else {
      executePower(signal);
        ...
        {isSuspended ? (
          ...
          <div className="space-y-2.5">
              ...
              {state === 'running' && (
                <>
                  ...
                  <button
                    ...
                    disabled={powerMut.isPending || isTransitioning}
                    className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl bg-blue-500 text-white text-sm font-semibold hover:bg-blue-600 transition-colors disabled:opacity-50"
                  >
                    {powerMut.isPending
                      ? <Loader2 className="w-4 h-4 animate-spin" />
            ...
            <button
              ...
              className="w-full py-2 rounded-xl border border-red-500/25 text-red-500 hover:bg-red-500/[0.05] text-xs font-medium transition-colors disabled:opacity-30 flex items-center justify-center gap-1.5"
            >
