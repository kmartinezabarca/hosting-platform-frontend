...
const PlanBillingCard = ({ service, plan }) => {
  ...
  const toggleAutoRenew = async () => {
    ...
    setAutoRenew(next); // UI optimista

    try {
      await apiClient.patch(`/api/services/${service.uuid}/auto-renew`, {
        auto_renew: next,
      });
    } catch (e) {
      console.error("No se pudo actualizar la renovación automática", e);
      setAutoRenew(!next); // revertir si falla
    } finally {
  ...
  return (
    ...
    >
      ...
      <div
        ...
        "
      >
        <h3 className="text-lg font-semibold text-foreground">Plan y Facturación</h3>
      ...
      <div className="p-4">
        ...
        <div className="mb-8">
          ...
          <InfoRow
            ...
            value={`$${service.price} ${(service?.currency ?? "").toString().toUpperCase()}`}
          />
          <InfoRow
          ...
          {/* Renovación Automática con toggle */}
          <div className="flex justify-between items-center pt-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <span
                ...
                ${autoRenew ? "bg-accent text-foreground" : "bg-muted text-foreground/70"}`}
              >
              ...
              <button
                ...
                `}
              >
                {savingAutoRenew && <Loader2 className="w-3.5 h-3.5 animate-spin" />}
        ...
        {/* Sección de Características del Plan (sub-card con borde + gradiente) */}
        {plan?.features && plan.features.length > 0 && (
          ...
          >
            ...
            {/* 1 col en móvil, 2 en md, 3 en xl (tu layout original) */}
            <ul role="list" className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-2">
