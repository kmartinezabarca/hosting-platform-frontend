...
function getCroppedImageBase64(image: HTMLImageElement, crop: PixelCrop, mimeType = 'image/jpeg') {
...
const BlogEditor = ({ content, onChange }) => {
  ...
  const [imageControls, setImageControls] = useState<{ top: number; left: number } | null>(null);
  ...
  /** Sanitiza el HTML antes de propagarlo hacia el padre / backend */
  ...
  const handleImageResize = (percentage) => {
    ...
    const currentValue = parseInt(selectedImage.getAttribute('width') as string) || 100;
            ...
            <div className="prose prose-sm max-w-none">
              <div dangerouslySetInnerHTML={{
                __html: DOMPurify.sanitize(editor?.getHTML() || '', {
                  // Permitir atributos de estilo/alineación usados por Tiptap
                  ADD_ATTR: ['class', 'style', 'target', 'rel'],
                  // Bloquear explícitamente scripts y atributos peligrosos
                  FORBID_TAGS: ['script', 'iframe', 'object', 'embed', 'form'],
                  FORBID_ATTR: ['onerror', 'onload', 'onclick', 'onmouseover'],
                })
              }} />
            ...
            {cropSrc && (
              <div className="w-full overflow-auto rounded-lg border bg-muted/30 p-2">
                <ReactCrop
                  ...
                  onComplete={(c) => setCompletedCrop(c ?? null)}
                  keepSelection
                >
                  <img
                    src={cropSrc}
