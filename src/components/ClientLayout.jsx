...
const ClientLayout = () => {
  ...
  useEffect(() => {
    const h = (e) => { if (profileRef.current && !profileRef.current.contains(e.target)) setIsProfileMenuOpen(false); };
  ...
  useEffect(() => {
    const h = (e) => { if (searchContainerRef.current && !searchContainerRef.current.contains(e.target)) setSearchFocused(false); };
  ...
  const handleSearchSelect = (href) => { navigate(href); setSearchQuery(''); setSearchFocused(false); searchInputRef.current?.blur(); };
  const handleLogout = async () => { try { await logout(); navigate('/login'); } catch (e) { console.error(e); } };
  const isActive = (href) => location.pathname === href || (href === '/client/dashboard' && location.pathname === '/client');
  const userName = [user?.first_name, user?.last_name].filter(Boolean).join(' ') || user?.name || 'Usuario';
